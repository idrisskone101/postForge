...
**shadcn/ui primitives** (`src/components/ui/*`) are the component layer. Motion is folded in via shared classes and tokenized Tailwind — no `components/motion/*`, no `motion` package, no `layoutId`, no parallel Button family.
