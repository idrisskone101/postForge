...
**Wraps:** All pages with Sidebar + TooltipProvider
...
- Theme: Dark by default, toggled via `localStorage.getItem("postforge-theme")`. Inline script avoids FOUC.
- Font: Poppins loaded via `next/font/google` with weights 400-800. Assigned to both `--font-sans` and `--font-mono`.
- Sidebar: Fixed left, 24px (w-24) on desktop, hidden on mobile (Sheet drawer).
- Main content: `ml-0 md:ml-24` to account for sidebar width.
- TooltipProvider wraps everything with `delay={0}`.
...
**Dependencies:** Button, Sheet/SheetContent/SheetTitle/SheetTrigger (ui), ThemeToggle, cn, lucide-react icons, next/link, next/navigation
...
- **Desktop:** Fixed pill-shaped icon sidebar (w-24, inner card w-20, rounded-[40px])
- **Mobile:** Sheet drawer triggered by hamburger menu button (fixed top-left)
- **Active detection:** Exact match for "/" root, startsWith for all other routes
- **Custom tooltips:** CSS-only tooltips on desktop hover (not using Tooltip component)
- **Design:** Uses cubic-bezier(0.34, 1.56, 0.64, 1) for bouncy spring animations
- **Nav items:** Each has unique accent color for hover/active states (blue, coral, green)
