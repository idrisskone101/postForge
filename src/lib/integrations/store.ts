...
function connectionKey(provider: IntegrationProvider) {
  return `integrations/connections/${provider}.json`;
}

function metricsKey(provider: IntegrationProvider) {
...
export async function readIntegrationConnection(
  ...
  const data = await storage.get(connectionKey(provider));
  if (!data) return null;
...
export async function saveIntegrationConnection(
  ...
  const context = `postforge:integration:${connection.provider}`;
  ...
  await storage.set(connectionKey(connection.provider), encode(stored));
}
...
export async function deleteIntegrationConnection(
  ...
  await Promise.all([
    storage.delete(connectionKey(provider)),
    storage.delete(metricsKey(provider)),
  ]);
}
...
export async function readProviderMetrics(
  ...
  const data = await storage.get(metricsKey(provider));
  if (!data) return null;
...
export async function saveProviderMetrics(
  ...
  await storage.set(metricsKey(record.provider), encode(record));
}
...
export async function deleteProviderMetrics(
  ...
  await storage.delete(metricsKey(provider));
}
