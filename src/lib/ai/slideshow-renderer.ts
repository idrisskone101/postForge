...
export type SlideshowRenderSlide = {
  id: string;
  eyebrow?: string;
  headline: string;
  body?: string;
...
export type SlideshowRenderTextSettings = {
  font?: "Poppins" | "Inter" | "Serif" | "Mono" | "Rounded" | string;
  color?: string;
  style?: "outline" | "solid" | "translucent" | "plain";
  size?: number;
...
const exportFontFamilies: Record<string, string> = {
...
function textOverlaySvg(
  ...
  const previewScale = Math.min(width, 1080) / 282;
  const headlineSize = Math.max(
    48,
    Math.min(220, height * 0.075, (settings.size ?? 28) * previewScale),
  );
  const bodySize = Math.max(22, Math.round(headlineSize * 0.46));
  const eyebrowSize = Math.max(18, Math.round(headlineSize * 0.34));
  ...
  const headlineHeight = headlineLines.length * headlineSize * 1.12;
  const bodyHeight = bodyLines.length ? bodyLines.length * bodySize * 1.45 + bodySize : 0;
  const eyebrowHeight = slide.eyebrow ? eyebrowSize * 2 : 0;
  const totalHeight = eyebrowHeight + headlineHeight + bodyHeight;
  const position = settings.position ?? "center";
  const top =
    position === "top"
      ? height * 0.13
      : position === "bottom"
        ? height - totalHeight - height * 0.13
        : (height - totalHeight) / 2;
  const align = settings.align ?? "center";
  const x = align === "left" ? (width - copyWidth) / 2 : align === "right" ? width - (width - copyWidth) / 2 : width / 2;
  const anchor = align === "left" ? "start" : align === "right" ? "end" : "middle";
  const color = textColor(settings.color);
  const fontFamily =
    exportFontFamilies[settings.font ?? "Poppins"] ?? exportFontFamilies.Poppins;
  const style = settings.style ?? "outline";
  const panelOpacity = style === "solid" ? 0.9 : style === "translucent" ? 0.56 : 0;
  ...
  const stroke = style === "outline" ? "#111111" : "none";
  const strokeWidth = style === "outline" ? Math.max(3, headlineSize * 0.08) : 0;

  let cursor = top;
  const chunks: string[] = [];
  if (slide.eyebrow) {
    chunks.push(
      `<text x="${x}" y="${cursor + eyebrowSize}" text-anchor="${anchor}" font-family="${fontFamily}" font-size="${eyebrowSize}" font-weight="700" letter-spacing="${eyebrowSize * 0.12}" fill="${color}" opacity=".84">${xml(slide.eyebrow.toUpperCase())}</text>`,
    );
    cursor += eyebrowHeight;
  }
  headlineLines.forEach((line, index) => {
    chunks.push(
      `<text x="${x}" y="${cursor + headlineSize * (index + 1)}" text-anchor="${anchor}" font-family="${fontFamily}" font-size="${headlineSize}" font-weight="800" fill="${color}" stroke="${stroke}" stroke-width="${strokeWidth}" paint-order="stroke" stroke-linejoin="round">${xml(line)}</text>`,
    );
  });
  cursor += headlineHeight + (bodyLines.length ? bodySize : 0);
  bodyLines.forEach((line, index) => {
    chunks.push(
      `<text x="${x}" y="${cursor + bodySize * 1.42 * (index + 1)}" text-anchor="${anchor}" font-family="${fontFamily}" font-size="${bodySize}" font-weight="500" fill="${color}" opacity=".94">${xml(line)}</text>`,
    );
  });
...
export async function renderSlideshowSlide(
  ...
  const { width, height } = aspectDimensions[options.aspectRatio ?? "9:16"] ?? aspectDimensions["9:16"];
  ...
  if (slide.displayText !== false) {
    layers.push({ input: textOverlaySvg(slide, width, height, options.textSettings ?? {}) });
  }
...
export function getSlideshowDimensions(aspectRatio: string) {
  return aspectDimensions[aspectRatio] ?? aspectDimensions["9:16"];
}
