  ...
  return (
    <div className="grid min-w-0 items-start gap-4 px-3 py-3 sm:px-5 sm:py-5 lg:px-8 xl:grid-cols-[minmax(360px,0.72fr)_minmax(500px,1.28fr)]">
      <div className="space-y-3">
        <div className="rounded-[7px] border border-[#DADBD2] bg-white p-4">
          <Skeleton className="mb-3 h-6 w-48 rounded-lg" />
          <Skeleton className="h-[118px] w-full rounded-[5px]" />
        ...
        <div className="rounded-[7px] border border-[#DADBD2] bg-white p-4">
          ...
          <Skeleton className="mb-3 h-11 w-full rounded-[6px]" />
          <div className="grid gap-2 sm:grid-cols-2 2xl:grid-cols-3">
            {Array.from({ length: 3 }).map((_, index) => (
              <Skeleton key={index} className="h-20 rounded-[5px]" />
            ))}
          </div>
        </div>
      ...
      <div className="overflow-hidden rounded-[8px] border border-[#DADBD2] bg-white">
        ...
        <div className="grid min-h-[590px] place-items-center bg-[#EFEFE9] p-8">
          <Skeleton className="h-[500px] w-[282px] max-w-full rounded-[7px]" />
        </div>
