...
export function CharactersPageClient() {
  ...
  return (
    <div className="px-5 py-5 sm:px-7 lg:px-8">
      ...
      {records.length === 0 ? (
        <section className="pf-card flex min-h-[650px] flex-col items-center justify-center px-5 text-center">
          ...
          <div className="mt-7 flex flex-wrap justify-center gap-2 text-[8px] text-[#858681]"><span className="rounded-full bg-[#F0F1EB] px-2.5 py-1">36 attribute groups</span><span className="rounded-full bg-[#F0F1EB] px-2.5 py-1">Prompt + JSON export</span><span className="rounded-full bg-[#F0F1EB] px-2.5 py-1">Database-backed</span></div>
        </section>
      ) : (
        <>
          ...
          {filtered.length === 0 ? <section className="mt-4 grid min-h-[430px] place-items-center rounded-xl border border-dashed border-[#CFCFC7] bg-[var(--pf-surface)] text-center"><div><Search className="mx-auto size-8 text-[#B0B1AC]" /><h3 className="mt-3 text-sm font-semibold">No matching characters</h3><p className="mt-1 text-[9px] text-[#858681]">Try a different name or identity filter.</p><button onClick={() => { setSearch(""); setGender("All identities"); }} className="pf-button-secondary mt-4">Clear filters</button></div></section> : <section className={cn("mt-4",view === "grid" ? "grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4" : "space-y-2")}>{filtered.map((record) => { const photoReady = Boolean(record.avatarId && record.previewKind === "photographic"); return <article key={record.id} className={cn("pf-card group relative overflow-hidden",view === "list" && "grid grid-cols-[74px_minmax(0,1fr)_auto] items-center p-2")}>
            <button onClick={() => setSelected(record)} className={cn("block w-full text-left",view === "grid" ? "" : "contents")}>
              ...
              <div className={cn("min-w-0",view === "grid" ? "p-3" : "px-3")}><div className="flex items-start justify-between gap-2"><div className="min-w-0"><h3 className="truncate text-[11px] font-semibold">{record.name}</h3><p className="mt-1 truncate text-[8px] text-[#858681]">{record.attributes.gender} · {record.attributes.age} · {record.attributes.ethnicity}</p></div>{view === "grid" && (photoReady ? <span className="pf-status-success shrink-0 px-2 py-1 text-[6px] font-bold">READY</span> : <span className="shrink-0 rounded-full bg-[#F0F1EB] px-2 py-1 text-[6px] font-bold text-[#777873]">DRAFT</span>)}</div>{view === "grid" && <div className="mt-3 flex min-w-0 flex-wrap gap-1.5"><span className="max-w-full truncate rounded-full bg-[#F0F1EB] px-2 py-1 text-[6px]">{record.attributes.hairStyle}</span><span className="max-w-full truncate rounded-full bg-[#F0F1EB] px-2 py-1 text-[6px]">{record.attributes.aesthetic}</span></div>}</div>
            </button>
            <div className={cn("relative",view === "grid" ? "absolute right-2 top-2" : "mr-2")}><button onClick={() => setMenu(menu === record.id ? null : record.id)} className="grid size-8 place-items-center rounded-full border border-white/70 bg-card/90 shadow-sm" aria-label={`Actions for ${record.name}`}><MoreHorizontal className="size-4" /></button>{menu === record.id && <div className="absolute right-0 top-9 z-20 w-36 rounded-[9px] border border-[#DADBD2] bg-white p-1.5 text-[9px] shadow-xl"><Link href={`/characters/new?id=${encodeURIComponent(record.id)}`} className="flex h-8 items-center gap-2 rounded-[6px] px-2 hover:bg-[#F0F1EB]"><Pencil className="size-3" /> Edit</Link><button onClick={() => duplicate(record)} className="flex h-8 w-full items-center gap-2 rounded-[6px] px-2 hover:bg-[#F0F1EB]"><Copy className="size-3" /> Duplicate</button><button onClick={() => remove(record)} className="flex h-8 w-full items-center gap-2 rounded-[6px] px-2 text-[#D94A34] hover:bg-[#FFF1EE]"><Trash2 className="size-3" /> Delete</button></div>}</div>
      ...
      {selected && <div className="pf-safe-overlay fixed inset-0 z-[70] grid place-items-center bg-black/45 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="character-detail-title" onClick={() => setSelected(null)}><div className="grid max-h-full w-full max-w-[780px] overflow-y-auto rounded-[16px] bg-white shadow-2xl md:grid-cols-[310px_1fr]" onClick={(event) => event.stopPropagation()}><div className="min-h-[380px] bg-[#CFD2CC]"><CharacterPhoto avatarId={selected.avatarId && selected.previewKind === "photographic" ? selected.avatarId : null} alt={`${selected.name} portrait`} className="min-h-[380px] md:h-full" /></div><div className="p-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="pf-eyebrow">{selected.avatarId && selected.previewKind === "photographic" ? "Saved identity" : "Draft character"}</p><h2 id="character-detail-title" className="mt-1 break-words text-xl font-semibold tracking-[-0.035em]">{selected.name}</h2><p className="mt-1 text-[9px] text-[#858681]">Updated {new Date(selected.updatedAt).toLocaleDateString()}</p></div><button onClick={() => setSelected(null)} className="grid size-8 shrink-0 place-items-center rounded-full border border-[#DADBD2]"><X className="size-3.5" /></button></div><div className="mt-5 grid grid-cols-2 gap-2">{Object.entries(selected.attributes).slice(0, 12).map(([key, value]) => <div key={key} className="min-w-0 rounded-[8px] bg-[#F3F4EF] p-2"><span className="block text-[6px] font-bold uppercase tracking-[.08em] text-[#999A95]">{key.replace(/([A-Z])/g, " $1")}</span><b className="mt-1 block truncate text-[8px]">{value}</b></div>)}</div><div className="mt-5 flex flex-wrap gap-2"><Link href={`/characters/new?id=${encodeURIComponent(selected.id)}`} className="pf-button-primary"><Pencil className="size-3.5" /> Edit character</Link><button onClick={() => copyPrompt(selected)} className="pf-button-secondary"><Copy className="size-3.5" /> Copy prompt</button><button onClick={() => remove(selected)} className="pf-button-secondary text-[#D94A34]"><Trash2 className="size-3.5" /> Delete</button></div></div></div></div>}
