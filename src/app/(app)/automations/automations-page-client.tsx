...
export function AutomationsPageClient({
  ...
  return (
    <div data-page-inset="true" className="px-5 py-5 sm:px-7 lg:px-8">
      ...
      {records.length === 0 ? <section className="pf-card pf-empty-stage flex min-h-[650px] min-w-0 flex-col items-center justify-center p-6 text-center"><div data-empty-stack="true"><div data-empty-deco="true" className="relative h-36 w-full min-w-0 max-w-72"><span className="absolute left-7 right-7 top-16 h-0.5 bg-[var(--pf-border)]" /><span data-empty-tile="true" className="absolute left-5 top-8 grid size-[66px] place-items-center rounded-lg border border-border bg-card text-[11px] font-bold text-[var(--pf-orange)] shadow-md">HOOK</span><span data-empty-tile="true" className="absolute left-1/2 top-8 grid size-[66px] -translate-x-1/2 place-items-center rounded-lg border border-border bg-card text-[11px] font-bold text-[var(--pf-link)] shadow-md">POST</span><span data-empty-tile="true" className="absolute right-5 top-8 grid size-[66px] place-items-center rounded-lg border border-border bg-card text-[var(--pf-success)] shadow-md"><Check className="size-4 shrink-0" /></span></div><h2 data-empty-heading="true" className="mt-4 text-[20px] font-semibold tracking-[-0.02em]">Build your first reviewed content plan</h2><p className="sr-only">Choose a playbook and save a schedule. Manual plans can create local review drafts; connected social plans can publish an approved Gallery video only after a separate review and explicit confirmation.</p><p aria-hidden="true" data-empty-copy="Choose a playbook and save a schedule. Manual plans can create local review drafts; connected social plans can publish an approved Gallery video only after a separate review and explicit confirmation." /><Link href="/automations/new" data-empty-actions="true" className="pf-button-primary mt-5"><Plus className="size-3.5 shrink-0" /> Create a content plan</Link><div data-empty-steps="true" className="mt-7 flex flex-col gap-2 text-left text-[11px] text-muted-foreground sm:flex-row sm:gap-6"><span><i className="mr-1.5 inline-grid size-5 place-items-center rounded-full bg-[var(--pf-active)] not-italic">1</i>Pick a playbook</span><span><i className="mr-1.5 inline-grid size-5 place-items-center rounded-full bg-[var(--pf-active)] not-italic">2</i>Choose a destination</span><span><i className="mr-1.5 inline-grid size-5 place-items-center rounded-full bg-[var(--pf-active)] not-italic">3</i>Save approval rules</span></div></div></section> : <>
