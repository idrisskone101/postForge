...
export function CollectionsPageClient({
  ...
  return (
    <div data-page-inset="true" className="px-5 py-5 sm:px-7 lg:px-8" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); upload(event.dataTransfer.files); }}>
      ...
      {assets.length === 0 && collections.length === 0 ? <section className="pf-card pf-empty-stage flex min-h-[650px] flex-col items-center justify-start p-6 text-center"><div data-empty-icon="true" className="grid size-14 place-items-center rounded-2xl bg-[var(--pf-active)] text-muted-foreground"><FolderOpen className="size-7" /></div><h2 data-empty-heading="true" data-empty-title="Your reusable image library lives here"><span className="sr-only">Your reusable image library lives here</span></h2><p className="sr-only">Upload owned product shots, portraits, locations, and textures. Group them into collections that other PostForge workflows can reuse.</p><p aria-hidden="true" data-empty-copy="Upload owned product shots, portraits, locations, and textures. Group them into collections that other PostForge workflows can reuse." /><div data-empty-actions="true" className="mt-5 flex flex-wrap items-center justify-center gap-2"><button onClick={() => inputRef.current?.click()} disabled={uploading} className="pf-button-primary">{uploading ? <Loader2 className="size-3.5 animate-spin" /> : <Upload className="size-3.5" />} Upload your first images</button><button onClick={() => setPinterestOpen(true)} className="pf-button-secondary"><Link2 className="size-3.5" /> Import from Pinterest</button></div><small data-empty-note="true" className="mt-3 text-[12px] text-muted-foreground">JPG, PNG, WEBP · up to 25 MB each</small></section> : <>
