  ...
  return (
    ...
    >
      ...
      {importOpen ? (
        ...
        >
          <div className="max-h-full min-w-0 w-full max-w-lg overflow-y-auto rounded-[8px] border border-[var(--pf-border)] bg-[var(--pf-surface)] p-5 shadow-[var(--pf-shadow-lg)]">
            <div className="flex min-w-0 items-start justify-between gap-3">
              <div className="min-w-0">
                <h2
                  id="import-character-title"
                  className="mt-1 break-words text-[15px] font-semibold text-[var(--pf-ink)]"
                >
                  Paste a prompt or attributes JSON
                </h2>
                <p className="mt-1 break-words text-[12px] leading-4 text-[var(--pf-muted)]">
                  Recognized fields are merged into the current character. Everything else is
                  ignored.
                </p>
              </div>
              <button
            ...
            <textarea
              ...
              className="mt-4 h-52 w-full min-w-0 resize-none rounded-[8px] border border-[var(--pf-border)] bg-[var(--pf-canvas)] p-3 font-mono text-[12px] leading-5 text-[var(--pf-ink)] outline-none focus:border-[var(--pf-orange)]"
              placeholder={'{"gender":"Female","age":"25-30","freckles":"Light Subtle"}'}
            />
            <div className="mt-4 flex flex-col-reverse gap-2 min-[420px]:flex-row min-[420px]:justify-end">
              <button type="button" onClick={() => setImportOpen(false)} className="pf-button-secondary">
                Cancel
              </button>
              <button type="button" onClick={applyImport} className="pf-button-primary">
                <Download className="size-3.5" /> Import attributes
              </button>
            </div>
          </div>
        </div>
